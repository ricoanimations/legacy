    ...
    if (url) {
        ...
        if (!url.match(/^https?:\/\/|^\//)) {
            ...
            if (url.indexOf('.paypal.com') !== -1) {
                debug(`startflow_paypalurl_with_token`, {
                    url
                });
            } else {
        ...
        if (paymentToken) {
            url = extendUrl(config.checkoutUrl, {
                token: paymentToken
            });
...
function awaitPaymentTokenAndUrl(event ? : ? Event, targetElement ? : ? HTMLElement): {
    url: ZalgoPromise < string > ,
    paymentToken: ZalgoPromise << ? string >
} {
...
function renderPayPalCheckout(props: Object = {}, hijackTarget ? : ? Element): ZalgoPromise < Object > {
    ...
    const errorHandler = once(err => {
        ...
        paymentToken.then(token => {
            ...
            return redirect(extendUrl(config.checkoutUrl, {
                token
            }));
        });
    });
...
function handleClickHijack(event, element): void {
    ...
    const {
        url,
        paymentToken
    } = awaitPaymentTokenAndUrl(event, targetElement);
    ...
    renderPayPalCheckout({
        url,
        payment: () => ZalgoPromise.resolve(token)
    }, targetElement);
}
...
function startFlow(item: string) {
    ...
    renderPayPalCheckout({
        url,
        payment: () => ZalgoPromise.resolve(paymentToken)
    });
}
