...
import {
    type Component
} from 'zoid/src/component/component';
import type {
        ...
        onClose: {
            ...
            decorate(original): Function {
                return function decorateOnClose(reason): ZalgoPromise < void > {
                    ...
                    const shouldCancel =
                        this.props.onCancel && [CLOSE_REASONS.CLOSE_DETECTED, CLOSE_REASONS.USER_CLOSED].indexOf(reason) !== -1;
...
if (Checkout.isChild() && Checkout.xchild && Checkout.xprops) {
    ...
    Checkout.xchild.onProps(xprops => {
        patchMethod(xprops, 'onAuthorize', ({
            callOriginal,
            args: [data]
        }) => {
            if (data && !data.intent) {
                warn(`hermes_authorize_no_intent`, {
                    paymentID: data.paymentID,
                    token: data.paymentToken
                });
