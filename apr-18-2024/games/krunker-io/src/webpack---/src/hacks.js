...
if (isIE() && getDomainSetting('ie_full_page')) {
    ...
    Checkout.renderTo = (win) => {
        ...
        Button.xprops.payment().then(token => {
            window.top.location = extendUrl(config.checkoutUrl, {
                token
            });
        }).catch(err => {
            checkout.error(err);
...
patchMethod(rest.payment, 'create', ({
    original: createOriginal,
    context: createContext,
    args: [env, client, options, experience]
}) => {
    if (!options.payment) {
        options = {
            payment: options,
...
patchMethod(Button.props.style, 'validate', ({
    callOriginal,
    args: [style]
}) => {
...
patchMethod(Button.props.payment, 'decorate', ({
    original,
    context,
    args: [originalPayment]
}) => {
    return original.call(context, function payment(data: Object, actions: Object): ZalgoPromise < string > {
        return new ZalgoPromise((resolve, reject) => {

            patchMethod(actions.payment, 'create', ({
                original: createOriginal,
                context: createContext,
                args: [options, experience]
            }) => {
                if (!options.payment) {
                    options = {
                        payment: options,
