...
import {
    DelegateComponent,
    type DelegateOptionsType
} from '../delegate';
import {
    isZoidComponentWindow,
    getComponentMeta
} from '../window';
import {
    CONTEXT_TYPES,
...
import {
    ...
    type BuiltInPropsDefinitionType,
    type PropsType,
    type BuiltInPropsType,
    type MixedPropDefinitionType
} from './props';
...
export class Component < P > extends BaseComponent < P > {
    ...
    listenDelegate() {
        ...
        on(`${ POST_MESSAGE.DELEGATE }_${ this.name }`, ({
            source,
            origin,
            data
        }) => {
    ...
    canRenderTo(win: CrossDomainWindowType): ZalgoPromise < boolean > {
        return send(win, `${ POST_MESSAGE.ALLOW_DELEGATE }_${ this.name }`).then(({
            data
        }) => {
            return data;
        }).catch(() => {
    ...
    getForEnv(item: (string | RegExp) | {
        [string]: (string | RegExp)
    }, env: ? string): ? (string | RegExp) {
    ...
    */
    ...
    init(props: (PropsType & P), context: ? string, element : ElementRefType): ParentComponent < P > {
        return new ParentComponent(this, this.getRenderContext(context, element), {
            props
        });
    }
    ...
    */
    ...
    render(props: (PropsType & P), element: ? ElementRefType): ZalgoPromise < ParentComponent < P >> {
        return ZalgoPromise.try(() => {
            return new ParentComponent(this, this.getRenderContext(null, element), {
                props
            }).render(element);
        });
    }
    ...
    renderIframe(props: (PropsType & P), element: ElementRefType): ZalgoPromise < ParentComponent < P >> {
        return ZalgoPromise.try(() => {
            return new ParentComponent(this, this.getRenderContext(CONTEXT_TYPES.IFRAME, element), {
                props
            }).render(element);
        });
    }
    ...
    renderPopup(props: (PropsType & P)): ZalgoPromise < ParentComponent < P >> {
        return ZalgoPromise.try(() => {
            return new ParentComponent(this, this.getRenderContext(CONTEXT_TYPES.POPUP), {
                props
            }).render();
        });
    }
    ...
    prerender(props: (PropsType & P), element: ElementRefType): {
        render: ((PropsType & P), ElementRefType) => ZalgoPromise < ParentComponent < P >> ,
        renderTo: (CrossDomainWindowType, (PropsType & P), ElementRefType) => ZalgoPromise < ParentComponent < P >>
    } {
        let instance = new ParentComponent(this, this.getRenderContext(null, element), {
            props
        });
        instance.prefetch();
    ...
    */
    ...
    logWarning(event: string, payload: {
        [string]: string
    }) {
