function triggerKeyboardEvent(e, t, n) {
    ...
    x.initEvent && x.initEvent(n, !0, !0), x.keyCode = t, x.which = t, e.dispatchEvent ? e.dispatchEvent(x) : e.fireEvent("onkeydown", x)
}
...
function score(e, t, n) {
    if (!(t <= 0)) {
        ...
        if (localStorage.highscore = Math.max(localStorage.highscore, data.score.amount += t), console.log(e.level), console.log(data.score.amount), n) {
...
function endLevel() {
    map.ending || (map.ending = !0, 1 == currentmap[0] && 1 == currentmap[1] && data.score.amount >= 25e3 && OpenModal(), map.random ? setMapRandom(["Random", "Castle"]) : setNextLevelArr(currentmap), storePlayerStats(), pause(), setMap())
}
...
function getGameScreen() {
    ...
    var e = 2;
    is_mobile && (e = 3), this.middlex = (this.left + this.right) / e, window.botmax = this.height - ceilmax, botmax < unitsize && (body.innerHTML = "<div><br>Your screen isn't high enough. Make it taller, then refresh.</div>"), this.deathheight = this.bottom + 48
}
...
function resetGameState(e) {
    clearAllTimeouts(), resetData(), window.nokeys = window.spawning = window.spawnon = window.notime = window.editing = window.qcount = window.lastscroll = 0, window.paused = window.gameon = window.speed = 1, e || (window.gamecount = 0), resetQuadrants(), window.gamehistory = [], window.sumLeftScroll = 0, window.firstMiddle = !1, AudioPlayer.pause()
}
...
function maintainPlayer(e) {
    ...
    if (player.alive) {
        ...
        player.xvel > 0 ? player.right > gamescreen.middlex && (t = !0, window.firstMiddle = !0, player.right > gamescreen.right - gamescreen.left && (player.xvel = min(0, player.xvel))) : player.left < 0 && (player.xvel = max(0, player.xvel)), player.under && (player.jumpcount = 0), window.scrolloffset = map.canscroll * (player.right - gamescreen.middlex), is_mobile ? (map.canscroll = !0, window.firstMiddle && !map.shifting && window.sumLeftScroll >= -220 || player.xvel > 0 && t ? (scrollWindow(lastscroll = round(min(player.scrollspeed, scrolloffset))), (window.sumLeftScroll <= 0 || window.sumLeftScroll > 0 && scrolloffset < 0) && (window.sumLeftScroll = window.sumLeftScroll + lastscroll)) : lastscroll = 0) : scrolloffset > 0 && !map.shifting ? scrollWindow(lastscroll = round(min(player.scrollspeed, scrolloffset))) : lastscroll = 0
    }
}
